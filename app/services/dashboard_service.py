...
class DashboardService:
    ...
    async def create(self, data: DashboardCreate) -> dict:
    ...
    async def update(self, dashboard_id: str, data: DashboardUpdate) -> dict:
        oid = self.validate_object_id(dashboard_id)
    ...
    async def delete(self, dashboard_id: str) -> dict:
        oid = self.validate_object_id(dashboard_id)
        existing = await self.collection.find_one({"_id": oid})
        if not existing:
            raise NotFoundException(resource="Dashboard", resource_id=dashboard_id)
